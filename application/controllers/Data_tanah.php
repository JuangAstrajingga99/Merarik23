    ...
    public function penduduk($id)
    {
        ...
        if ($data_tanah) {
            foreach ($data_tanah as $p) {
                ...
                $marker['infowindow_content'] = 'Wilayah : '.$p->nama.'<p>Harga Tiket : '.$p->persil.'</p><p>Luas Wilayah :'.$p->luas_tanah.' m2</p><img src="'.base_url().'gambar/tanah/'.$p->gambar.'" width="200px" heigt="200px"><br><br><a href="'.site_url().'data_tanah/update/'.$p->id_tanah.'" class="btn btn-warning btn-sm"> <i class="fa fa-edit"> </i> Edit</a><a href="'.site_url().'data_tanah/delete/'.$p->id_tanah.'/'.$p->id_penduduk.'" class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Are You Sure ?\')"><i class="fa fa-trash"> </i> Delete</a>';
